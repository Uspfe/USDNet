...
class HungarianMatcher(nn.Module):
    ...
    def __init__(
        ...
        cost_origin: float = 0,
        cost_axis: float = 0,
        cost_ce_inter_mask: float = 0,
        cost_dice_inter_mask: float = 0,
        cost_inter_centers: float = 0,
        cost_mov_centers: float = 0,
    ):
        """Creates the matcher
        ...
        self.cost_ce_inter_mask = cost_ce_inter_mask
        self.cost_dice_inter_mask = cost_dice_inter_mask
        self.cost_inter_centers = cost_inter_centers
        self.cost_mov_centers = cost_mov_centers
        assert (
    ...
    def memory_efficient_forward(self, outputs, targets, mask_type):
        ...
        for b in range(bs):
            ...
            # print("     tgt_ids.shape: {}, tgt_ids.max: {}, tgt_ids.min: {}".format(tgt_ids.shape, tgt_ids.max(), tgt_ids.min()))
            # print("     out_prob.shape: {}, out_prob.max: {}, out_prob.min: {}".format(out_prob.shape, out_prob.max(), out_prob.min()))
            ...
            #     align_corners=False,
            # ).squeeze(1)
            # print("     out_mask.shape: {}, tgt_mask.shape: {}".format(out_mask.shape, tgt_mask.shape))
