...
def load_interaction_backbone_from_checkpoint(cfg, model):
    state_dict = torch.load(cfg.general.interaction_backbone_checkpoint)["state_dict"]
    correct_dict = dict(model.state_dict())

    # if parametrs not found in checkpoint they will be randomly initialized
    for key_state in state_dict.keys():
        if "model.backbone." in key_state:
            key_correct = key_state.replace("model.backbone.", "model.interaction_backbone.")
            if key_correct not in correct_dict.keys():
                logger.warning(
                    f"Key not found, it will be initialized randomly: {key_correct}"
                )

    # if parametrs have different shape, it will randomly initialize
    # state_dict = torch.load(cfg.general.interaction_backbone_checkpoint)["state_dict"]
    # correct_dict = dict(model.state_dict())
    for key_correct in correct_dict.keys():
        if "model.interaction_backbone." in key_correct:
            key_state = key_correct.replace("model.interaction_backbone.", "model.backbone.")
            if key_state not in state_dict:
                logger.warning(f"{key_state} not in loaded checkpoint")
                state_dict.update(
                    {key_state: correct_dict[key_correct]}
                )
            elif (
                state_dict[key_state].shape
                != correct_dict[key_correct].shape
            ):
                logger.warning(
                    f"incorrect shape {key_state}:{state_dict[key_state].shape} vs {correct_dict[key_correct].shape}"
                )
                state_dict.update({key_state: correct_dict[correct_dict]})

    # if we have more keys just discard them
    # correct_dict = dict(model.state_dict())
    new_correct_dict = dict(model.state_dict())
    for key_state in state_dict.keys():
        if "model.backbone." in key_state:
            key_correct = key_state.replace("model.backbone.", "model.interaction_backbone.")
            # logger.warning("   loading {} from checkpoint {}".format(key_correct, key_state))
            assert key_correct in correct_dict.keys()
            new_correct_dict.update({key_correct: state_dict[key_state]})
        model.load_state_dict(new_correct_dict)
    return cfg, model
